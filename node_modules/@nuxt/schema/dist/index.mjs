...
const experimental = defineResolvers({
  ...
  features: {
    ...
    noScripts: {
      async $resolve(val, get) {
        return typeof val === "boolean" ? val : await get("experimental").then((e) => e?.noScripts) ?? false;
      }
    }
  },
  experimental: {
    ...
    emitRouteChunkError: {
      $resolve: (val) => {
        ...
        const validOptions = ["manual", "automatic", "automatic-immediate"];
    ...
    watcher: {
      $resolve: async (val, get) => {
        const validOptions = ["chokidar", "parcel", "chokidar-granular"];
        if (typeof val === "string" && validOptions.includes(val)) {
          return val;
        }
        const [srcDir, rootDir] = await Promise.all([get("srcDir"), get("rootDir")]);
    ...
    spaLoadingTemplateLocation: {
      $resolve: async (val, get) => {
        const validOptions = ["body", "within"];
        return typeof val === "string" && validOptions.includes(val) ? val : (await get("future")).compatibilityVersion === 4 ? "body" : "within";
      }
    },
    /**
...
const typescript = defineResolvers({
  ...
  typescript: {
    ...
    builder: {
      $resolve: (val) => {
        const validBuilderTypes = ["vite", "webpack", "rspack", "shared"];
        if (typeof val === "string" && validBuilderTypes.includes(val)) {
          return val;
        }
        if (val === false) {
          return false;
